...
const PROJECTS = [
  ...
  { title: 'ClipKeep',           subtitle: 'Social clip vault · collections & search', year: '2025', slug: 'clipkeep', image: '/previews/clipkeep/screenshot-1.webp' },
...
/** Same breakpoint as HeroCarousel — no wheel-driven “fly away” on phones */
...
export default function HeroAlt() {
  ...
  useEffect(() => {
    ...
    const getPushDown = () => {
      ...
      if (w >= 1024) return w * 0.055
      if (w >= 768) return w * 0.04
      return w * 0.028
    }
    gsap.set(wrapper, { y: getPushDown() })
    ...
    const onResize = () => {
      ...
      resizeT = setTimeout(() => {
        if (wrapper) gsap.set(wrapper, { y: getPushDown() })
      }, 120)
    }
  ...
  // Wheel: blocked during entrance; drives scroll-exit + carousel fade (desktop only — mobile keeps hero stable while page scrolls)
  ...
  return (
    <div
      onWheel={handleWheel}
      className="relative w-full min-h-[120dvh] max-h-none md:min-h-[100dvh] md:max-h-[100dvh] bg-[#130502] text-[#fff7f7] overflow-y-hidden flex flex-col"
    >

      <SiteMenuCard ref={menuRef} />

      {/* Mobile: Siftion first (order-1), projects below (order-2). md+: carousel fills above wordmark. */}
      {/* ─── Siftion wordmark ───────────────────────────────────────── */}
      <div
        ref={svgWrapperRef}
        className="order-1 md:order-2 w-full shrink-0 px-4 sm:px-8 md:px-12 lg:px-[60px] pt-20 md:pt-0 pb-3 md:pb-[max(0.25rem,env(safe-area-inset-bottom))]"
      ...
      <div
        ref={carouselRef}
        className="hero-alt-carousel order-2 md:order-1 flex-1 flex flex-col justify-start md:justify-end min-h-0 pb-[env(safe-area-inset-bottom,0px)]"
      >
        <HeroCarousel items={PROJECTS} showHeader={false} seamless />
      </div>

      {/* Caption overrides for dark background */}
      <style>{`
        .hero-alt-carousel .caption-title    { color: #fff7f7; }
        .hero-alt-carousel .caption-subtitle { color: rgba(246,232,234,0.55); }
        .hero-alt-carousel .carousel-item-media {
          border-color: rgba(246,232,234,0.10);
          background:   rgba(246,232,234,0.02);
        }
      `}</style>
    </div>
  )
