  ...
  return (
    <main className="min-h-screen bg-[var(--background)] text-[var(--foreground)] antialiased overflow-x-hidden">
      ...
      <section className="px-[6%] pt-[118px] md:pt-[146px] pb-14 border-b border-white/12">
        ...
        <p className="mt-6 max-w-[900px] text-white/72 leading-relaxed">
      ...
      <section className="px-[6%] py-12 border-b border-white/12">
        <div className="space-y-4">
          {pulses.map((p, i) => (
            ...
            >
              <Link href={`/signals/${p.slug}`} className="no-underline text-white">
                <p className="text-[11px] uppercase tracking-[0.18em] text-white/45">Lumé's Signal · {new Date(p.timestamp).toLocaleDateString()}</p>
                <h2 className="mt-2 text-3xl tracking-[-0.03em] hover:text-[var(--accent)] transition-colors">{p.title}</h2>
                <p className="mt-3 text-white/68 line-clamp-2">{p.content}</p>
              </Link>
            </motion.article>
          ))}

          {!pulses.length && (
            <div className="rounded-2xl border border-white/12 bg-white/[0.03] p-6 text-white/65">
              Signals are syncing. Check back in a moment.
            </div>
          )}
        </div>
      </section>
